...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out2_dx, out2_dw, out2_db = affine_backward(dscores, cache2)
        grads['W2'] = out2_dw + self.reg * self.params['W2']
        grads['b2'] = out2_db

        out1_dx, out1_dw, out1_db = affine_relu_backward(out2_dx,cache)
        grads['W1'] = out1_dw + self.reg * self.params['W1']
        grads['b1'] = out1_db
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
            if self.use_batchnorm:
                ...
                key_beta = 'beta'+str(i)
                actual_input, cache[i] = affine_bn_relu_forward(actual_input, self.params['W' + str(i)], self.params['b' + str(i)],
self.params[key_gamma], self.params[key_beta], self.bn_params[i-1] )
            else:
                actual_input, cache[i] = affine_relu_forward(actual_input, self.params['W' + str(i)], self.params['b' + str(i)])
            if self.use_dropout:
                actual_input, dropout_cache[i] = dropout_forward(actual_input,self.dropout_param)
        # For all minus the last one
        # Now for the last one
        out_lay, cache_last = affine_forward(actual_input, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        scores = out_lay
        ...
        ############################################################################
        loss, dscores = softmax_loss(scores, y)
        affine_dx, affine_dw, affine_db = affine_backward(dscores, cache_last)
        ...
        # For the last layer then for the other ones
        for i in range(self.num_layers-1,0,-1):
            if self.use_dropout:
                affine_dx = dropout_backward(affine_dx, dropout_cache[i])
            if self.use_batchnorm:
                affine_dx, affine_dw, affine_db, dgamma, dbeta = affine_bn_relu_backward(affine_dx, cache[i])
