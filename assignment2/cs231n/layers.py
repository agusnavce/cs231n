...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for c in range(C): # with these for's I manage to get a layer of the nth input and pad it
            x_pad[n,c] = np.pad(x[n,c],(pad,pad),'constant', constant_values=(0,0))

    for n in range(N): # for each input
        for i in range(H_out):
            for j in range(W_out): # in both directions
                for f in range(F): # for each filter
                    # choose the matrix to convolute with all the channels
                    matrix = x_pad[n,:, i*stride: i*stride+HH, j*stride:j*stride+WW]
                    # choose filter in this iteration
                    current_filter = w[f]
                    # do the convolution
                    out[n,f,i,j] = np.sum(matrix*current_filter)
                # sum the biases for all
...
def leakyrelu_forward(x,config=None):
    """
    Computes the forward pass for a layer of rectified linear units (ReLUs).
    ...
    config.setdefault('leak', 0.01)
    out = None
    ###########################################################################
    # TODO: Implement the ReLU forward pass.                                  #
    ###########################################################################
    out = np.maximum(0,x) + config['leak']* np.minimum(0,x)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = x
...
def prelu_backward(dout, cache):
    ...
    - dx: Gradient with respect to x
    """
    dx ,da ,x, alpha = None ,None ,cache


    ###########################################################################
    # TODO: Implement the PReLU backward pass.  based on Surpassing           #
    #  Human-Level Performance on ImageNet Classification paper               #
    ###########################################################################
    dx = np.copy(dout)
    dx[x<0] = alpha
...
def maxout_forward(x ,alpha ,beta):
    """
    Computes the forward pass for a layer of maxout .

    Inputs:
    - x: A numpy array containing input data, of shape (N, d_1, ..., d_k)
    - alpha: A numpy array of weights, of shape (D, M, K)
    - beta: A numpy array of biases, of shape (M, K)
    ...
    N = x.shape[0]
    x_reshaped = x.reshape(N,-1)
    h1 = x_reshaped.dot(alpha) + beta
    out = np.amax(h1, axis=2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, alpha, beta)
...
def maxout_backward(dout, cache):
    ...
    - dx: Gradient with respect to x
    """
    dx ,dalpha ,dbeta, x, alpha, beta = None ,None ,cache
